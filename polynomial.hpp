...
namespace polynomial {
    ...
    inline auto root_rational_candidates(polynomial<T> p) -> std::set<T> {
        ...
        for (std::size_t i = 1; i <= leading_quotient; i++) {
            if (leading_quotient % i == 0) { factor_of_last_term.push_back(i); }
        }
        for (std::size_t i = 1;
             i <= static_cast<std::size_t>(p.coefficients[0]);
             i++) {
            if (static_cast<int>(p.coefficients[0]) % static_cast<int>(i) ==
                0) {
    ...
    template <typename T>
    auto gcd(polynomial<T> p,
             polynomial<T> q) -> std::pair<polynomial<T>, std::vector<T>> {
        std::vector<T> steps;
...
void performoperation(const polynomial::polynomial<T> &p1,
    ...
    for (auto coeff : result.coefficients) { std::cout << coeff << " "; }
    std::cout << std::endl;
}
...
void example() {
    ...
    p1.degree = 2;
    p2.degree = 1;
    std::cout << "This program performs operations on two polynomials"
              << std::endl;
    ...
    double value = 2.0;
    std::cout << "Evaluation at x = " << value << ": " << p1(value)
              << std::endl;
